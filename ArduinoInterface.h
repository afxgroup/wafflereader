...
namespace ArduinoFloppyReader {
	...
	private:
		...
		DiagnosticResponse internalOpenPort(const std::string& portName, bool enableCTSflowcontrol, bool triggerReset, std::string& versionString);

		// Attempt to sync and get version
		DiagnosticResponse attemptToSync(std::string& versionString);
	...
	public:
		...
		LastCommand getLastFailedCommand() const { return m_lastCommand; }
		DiagnosticResponse getLastError() const { return m_lastError; }
		...
		bool isOpen() const { return m_comPort->isPortOpen(); }
		bool isInWriteMode() const { return m_inWriteMode; }

		// Returns a list of ports this could be available on
		static void enumeratePorts(std::vector<std::string>& portList);
		...
		DiagnosticResponse openPort(const std::string& portName, bool enableCTSflowcontrol = true);

		// Attempts to verify if the reader/writer is running on this port
		bool isPortCorrect(const std::string& portName);
