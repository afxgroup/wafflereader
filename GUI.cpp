...
int main()
{
    ...
    const char *floatingText = GetString(MSG_FLOATING_TEXT);
    ...
    while (!WindowShouldClose() || isWorking) // Detect window close button or ESC key
    {
        if (portNumbers > 0)
        {
            if (!isWorking)
            {
                GuiEnable();
                if (loadF2DFile)
                    GetASLFilename(GetString(MSG_FILE_TO_DISK), fileNameWrite, PATH_MAX, FALSE);
                if (loadD2FFile)
                    GetASLFilename(GetString(MSG_DISK_TO_FILE), fileNameRead, PATH_MAX, TRUE);
                if (isWriting)
                {
        ...
        {
            DrawTextEx(topazFont, LS(SELECT_ADF_WRITE), (Vector2){22.0, 176.0}, 13, 1, GRAY);
        }

        if (GuiButton((Rectangle){310, 170, 25, 25}, "..."))
            loadF2DFile = true;

        if (GuiButton((Rectangle){120, 300, 105, 20}, LS(START_WRITE)))
            isWriting = true;

        if (GuiButton((Rectangle){120, 450, 105, 20}, LS(START_READ)))
            isReading = true;

        GuiCheckBox((Rectangle){20, 275, 15, 15}, LS(VERIFY), &verify);
        GuiCheckBox((Rectangle){260, 275, 15, 15}, LS(PCW), &pcw);
        ...
        {
            DrawTextEx(topazFont, LS(SELECT_FILE_READ), (Vector2){22.0, 336.0}, 13, 1, GRAY);
        }

        if (GuiButton((Rectangle){310, 330, 25, 25}, "..."))
            loadD2FFile = true;

        if (GuiCheckBox((Rectangle){20, 452, 15, 15}, LS(TRACKS_82), &tracks82))
            tracks82 = !tracks82;
        if (GuiCheckBox((Rectangle){650, 515, 15, 15}, LS(HD_DISK_SELECTION), &hdSelection))
            hdSelection = !hdSelection;
        GuiLabel((Rectangle){70, 510, 150, 25}, LS(WAFFLE_DRIVE_PORT));
        ...
        if (isWorking)
        {
            if (GuiEnabledButton((Rectangle){screenWidth / 2 - 60, 510, 120, 25}, LS(STOP)))
                stopWorking = true;
        }
