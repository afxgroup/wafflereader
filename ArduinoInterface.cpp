...
std::string lastCommandToName(LastCommand cmd)
{
	switch (cmd)
	{
	case LastCommand::lcOpenPort:
		return "OpenPort";
	case LastCommand::lcGetVersion:
		return "GetVersion";
	case LastCommand::lcEnableWrite:
		return "EnableWrite";
	case LastCommand::lcRewind:
		return "Rewind";
	case LastCommand::lcDisableMotor:
		return "DisableMotor";
	case LastCommand::lcEnableMotor:
		return "EnableMotor";
	case LastCommand::lcGotoTrack:
		return "GotoTrack";
	case LastCommand::lcSelectSurface:
		return "SelectSurface";
	case LastCommand::lcReadTrack:
		return "ReadTrack";
	case LastCommand::lcWriteTrack:
		return "WriteTrack";
	case LastCommand::lcRunDiagnostics:
		return "RunDiagnostics";
	case LastCommand::lcSwitchDiskMode:
		return "SetCapacity";
	case LastCommand::lcReadTrackStream:
		return "ReadTrackStream";
	case LastCommand::lcCheckDiskInDrive:
		return "CheckDiskInDrive";
	case LastCommand::lcCheckDiskWriteProtected:
		return "CheckDiskWriteProtected";
	case LastCommand::lcEraseTrack:
		return "EraseTrack";
	case LastCommand::lcNoClickCheck:
		return "NoClickCheck";
	case LastCommand::lcCheckDensity:
		return "CheckDensity";
	case LastCommand::lcMeasureRPM:
		return "MeasureRPM";
	case LastCommand::lcEEPROMRead:
		return "EEPROM Read";
	case LastCommand::lcEEPROMWrite:
		return "EEPROM Write";
	case LastCommand::lcWriteFlux:
		return "Write Flux";
	case LastCommand::lcEraseFlux:
		return "Erase Flux";
	default:
		return "Unknown";
	}
...
const std::string ArduinoInterface::getLastErrorStr() const
{
	...
	switch (m_lastError)
	{
	case DiagnosticResponse::drOldFirmware:
	...
	case DiagnosticResponse::drPortInUse:
		return "The specified COM port is currently in use by another application.";
	case DiagnosticResponse::drPortNotFound:
		return "The specified COM port was not found.";
	case DiagnosticResponse::drAccessDenied:
		return "The operating system denied access to the specified COM port.";
	case DiagnosticResponse::drComportConfigError:
		return "We were unable to configure the COM port using the SetCommConfig() command.";
	case DiagnosticResponse::drBaudRateNotSupported:
		return "The COM port does not support the 2M baud rate required by this application.";
	case DiagnosticResponse::drErrorReadingVersion:
		return "An error occurred attempting to read the version of the sketch running on the Arduino.";
	case DiagnosticResponse::drErrorMalformedVersion:
		return "The Arduino returned an unexpected string when version was requested.  This could be a baud rate mismatch or incorrect loaded sketch.";
	case DiagnosticResponse::drCTSFailure:
		return "Diagnostics reports the CTS line is not connected correctly or is not behaving correctly.";
	case DiagnosticResponse::drTrackRangeError:
		return "An error occurred attempting to go to a track number that was out of allowed range.";
	case DiagnosticResponse::drWriteProtected:
		return "Unable to write to the disk.  The disk is write protected.";
	case DiagnosticResponse::drPortError:
		return "An unknown error occurred attempting to open access to the specified COM port.";
	case DiagnosticResponse::drDiagnosticNotAvailable:
		return "CTS diagnostic not available, command GetCommModemStatus failed to execute.";
	case DiagnosticResponse::drSelectTrackError:
		return "Arduino reported an error seeking to a specific track.";
	case DiagnosticResponse::drTrackWriteResponseError:
		return "Error receiving status from Arduino after a track write operation.";
	case DiagnosticResponse::drSendDataFailed:
		return "Error sending track data to be written to disk.  This could be a COM timeout.";
	case DiagnosticResponse::drRewindFailure:
		return "Arduino was unable to find track 0.  This could be a wiring fault or power supply failure.";
	case DiagnosticResponse::drNoDiskInDrive:
		return "No disk in drive";
	case DiagnosticResponse::drMediaTypeMismatch:
		if (m_lastCommand == LastCommand::lcWriteFlux)
			return "Write Flux only supports DD disks and data";
		else
			return "An attempt to read or write had an incorrect amount of data given the DD/HD media used";
	case DiagnosticResponse::drWriteTimeout:
		return "The Arduino could not receive the data quick enough to write to disk. Try connecting via USB2 and not using a USB hub.\n\nIf this still does not work, turn off precomp if you are using it.";
	case DiagnosticResponse::drFramingError:
		return "The Arduino received bad data from the PC. This could indicate poor connectivity, bad baud rate matching or damaged cables.";
	case DiagnosticResponse::drSerialOverrun:
		return "The Arduino received data faster than it could handle. This could either be a fault with the CTS connection or the USB/serial interface is faulty";
	case DiagnosticResponse::drUSBSerialBad:
		return "The USB->Serial converter being used isn't suitable and doesn't run consistently fast enough.  Please ensure you use a genuine FTDI adapter.";
	case DiagnosticResponse::drError:
		tmp << "Arduino responded with an error running the " << lastCommandToName(m_lastCommand) << " command.";
		return tmp.str();
	...
	case DiagnosticResponse::drReadResponseFailed:
		...
		{
		case LastCommand::lcGotoTrack:
			return "Unable to read response from Arduino after requesting to go to a specific track";
		case LastCommand::lcReadTrack:
			return "Gave up trying to read a full track from the disk.";
		case LastCommand::lcWriteTrack:
			return "Unable to read response to requesting to write a track.";
		default:
			tmp << "Error reading response from the Arduino while running command " << lastCommandToName(m_lastCommand) << ".";
			return tmp.str();
		}
	...
	case DiagnosticResponse::drSendParameterFailed:
		tmp << "Unable to send a parameter while executing the " << lastCommandToName(m_lastCommand) << " command.";
		return tmp.str();
	case DiagnosticResponse::drStatusError:
		tmp << "An unknown response was was received from the Arduino while executing the " << lastCommandToName(m_lastCommand) << " command.";
		return tmp.str();
...
bool ArduinoInterface::isPortCorrect(const std::string &portName)
{
	// Attempts to verify if the reader/writer is running on this port
	std::string version;
	DiagnosticResponse dr = internalOpenPort(portName, false, true, version);
	m_comPort->closePort();
	return dr == DiagnosticResponse::drOK;
}

// Attempt to sync and get version
DiagnosticResponse ArduinoInterface::attemptToSync(std::string &versionString)
{
	char buffer[10] = {0};
	...
	unsigned long size = m_comPort->write(&buffer[0], 3);
	if (size != 3)
	{
	...
	for (;;)
	{
		...
		size = m_comPort->read(&buffer[4], 1);
		bytesRead += size;
		// Was something read?
		if (size)
		{
		...
		else
		{
			...
			if (counterNoData % 7 == 6 && bytesRead == 0)
			{
				...
				size = m_comPort->write(&buffer[0], 1);
				if (size != 1)
				{
					// Couldn't write to device
					m_comPort->closePort();
...
// Attempts to verify if the reader/writer is running on this port
DiagnosticResponse ArduinoInterface::internalOpenPort(const std::string &portName, bool enableCTSflowcontrol, bool triggerReset, std::string &versionString)
{
	switch (m_comPort->openPort(portName))
	{
	case SerialIO::Response::rInUse:
		return DiagnosticResponse::drPortInUse;
	case SerialIO::Response::rNotFound:
		return DiagnosticResponse::drPortNotFound;
	case SerialIO::Response::rOK:
		break;
	default:
		return DiagnosticResponse::drPortError;
	}
	...
	if (m_comPort->configurePort(config) != SerialIO::Response::rOK)
	{
		m_comPort->closePort();
		return DiagnosticResponse::drPortError;
	}

	m_comPort->setBufferSizes(16, 16);
	m_comPort->setReadTimeouts(10, 250);
	m_comPort->setWriteTimeouts(2000, 200);

	// Try to get the version
	DiagnosticResponse response = attemptToSync(versionString);
	if (response != DiagnosticResponse::drOK)
	{
		...
		if (triggerReset)
		{
			...
			m_comPort->setDTR(true);
			m_comPort->setRTS(true);
			std::this_thread::sleep_for(std::chrono::milliseconds(10));
			m_comPort->closePort();
			std::this_thread::sleep_for(std::chrono::milliseconds(150));

			// Now re-connect and try again
			if (m_comPort->openPort(portName) != SerialIO::Response::rOK)
				return DiagnosticResponse::drPortError;

			response = attemptToSync(versionString);
			if (response != DiagnosticResponse::drOK)
			{
				m_comPort->closePort();
				return response;
...
DiagnosticResponse ArduinoInterface::openPort(const std::string &portName, bool enableCTSflowcontrol)
{
	...
	m_lastError = internalOpenPort(portName, enableCTSflowcontrol, true, versionString);
	if (m_lastError != DiagnosticResponse::drOK)
		return m_lastError;
...
DiagnosticResponse ArduinoInterface::findTrack0()
{
	...
	{
		if (status == '#')
			return DiagnosticResponse::drRewindFailure;
	}
	return m_lastError;
}
...
{
	if (dbTime <= FLUX_REPEAT_BLANK_DB)
		return dbTime + FLUX_MINIMUM_DB;
...
void ArduinoInterface::enumeratePorts(std::vector<std::string> &portList)
{
	portList.clear();
	...
	for (const SerialIO::SerialPortInformation &port : prts)
	{
		...
		if ((port.vid == 0x1209) && (port.pid == 0x4d69))
			continue;
		if ((port.vid == 0x1209) && (port.pid == 0x0001))
			continue;
		if (port.productName == "Greaseweazle")
			continue;
		if (port.instanceID.find("\\GW") != std::string::npos)
			continue;

		// Skip Supercard pro
		if (port.portName.find("SCP-JIM") != std::string::npos)
			continue;
		if (port.portName.find("Supercard Pro") != std::string::npos)
			continue;
...
{
	unsigned int bytes = m_comPort->write(source, numBytes);
